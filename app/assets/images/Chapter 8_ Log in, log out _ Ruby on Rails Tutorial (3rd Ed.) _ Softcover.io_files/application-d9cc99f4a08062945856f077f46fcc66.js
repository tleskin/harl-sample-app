...
(function() {
  window.BookManage = {
    init: function(book, discounts) {
      ...
      $("#addDiscountBtn").click((function(_this) {
        return function() {
          ...
          $(".j_discountFormEdit").hide();
          return _this.scrollToEdit();
        };
      })(this));
...
(function() {
  window.Book = {
    ...
    init: function(opts) {
      ...
      this.opts = opts;
      this.custom_math = $.parseJSON("{" + this.opts.custom_math + "}");
      ...
      return $(window).scroll((function(_this) {
        return function() {
          ...
          if ($(window).scrollTop() > $(".container").outerHeight() * 0.6 && (_this.slug === "ruby_on_rails_tutorial" || _this.slug === "ruby_on_rails_tutorial_3rd_edition") && _this.chapter.slug !== 'frontmatter') {
    ...
    getChapter: function(slug, offset) {
      ...
      return $.get(this.chapter.s3_url, (function(_this) {
        return function(result) {
          ...
          $('#bookHtml .hyperref, #bookHtml .footnote a, #bookHtml .arrow').each(function(i, a) {
    ...
    initMathJax: function(cb) {
      ...
      return script.src = 'https://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS_HTML';
    }
  };
      ...
      return $((function(_this) {
        return function() {
          if (_this.opts.header) {
            ...
            if (_this.opts.author) {
              $('.container').addClass("authorView");
            }
            return setupMenus();
          }
        };
  ...
  window.Editor = {
    init: function(opts) {
      ...
      setInterval(function() {
        if (editorWidth !== $('#editorPane').width()) {
          editorWidth = $('#editorPane').width();
          return $('#previewPane').width(($(window).width() - editorWidth) - 81);
        }
      }, 50);
      log('connecting...');
      this.socket = io(opts.socketServer);
      this.socket.on('welcome', (function(_this) {
        return function() {
          return log('connected');
        };
      })(this));
      this.socket.on('html', (function(_this) {
        return function(data) {
          log("took " + ((new Date() - _this.previewStartedAt) / 1000.0) + "s");
          return _this.setContent(data.content);
        };
      })(this));
      $('#content').bind('keydown', 'ctrl+s', (function(_this) {
        return function() {
          return _this.preview();
        };
      })(this));
      return $('#content').scroll(function() {
        if (!($('#content').scrollTop() > 35)) {
          return $('#content').css('paddingTop', 50 - (2.5 * $('#content').scrollTop()));
        }
      });
    },
    getContent: function() {
    ...
    setContent: function(html) {
      return $("#previewPane #book")[0].innerHTML = html;
    },
    preview: function() {
      this.setContent("previewing...");
      this.previewStartedAt = new Date();
      return this.socket.emit('preview', {
...
(function() {
  window.Purchase = {
    ...
    calculatePrice: function() {
      ...
      $(".j_checkoutTotal").html("$" + (this.total.toFixed(2)));
      return $("#j_cc_form").toggle(this.total > 0);
    },
    ...
    submitForm: function() {
      ...
      if (((_ref = $("#purchaseEmail").val()) != null ? _ref.trim() : void 0) !== ((_ref1 = $("#purchaseEmailConfirmation").val()) != null ? _ref1.trim() : void 0)) {
      ...
      this.disableSubmit();
      if (this.useCardOnFile || this.total === 0) {
...
(function() {
  ...
  window.marketingCTA = function() {
    ...
    if ($('#buy').length) {
      ctaShow = $('#buy').offset().top;
      buyShow = $('#buy').offset().top + ($('#buy').outerHeight() / 2);
    } else {
  ...
  window.closeEmailPops = function() {
    $('.emailPitch').removeClass('open').hide();
    $("#bookMenu").removeClass('open');
    $('#dropBG').removeClass('open').hide();
    return localStorage.setItem('emailClose', 'yes');
  };
...
(function() {
  window.Wizard = {
    init: function(book_id, attrs) {
      var els;
      els = $(".authorWizardSetup input");
      return els.each(function(n, el) {
        $(el).change(function(e) {
          var checked, new_attrs;
          checked = $(el).is(':checked');
          $("." + el.id + "-yes").toggle(checked);
          $("." + el.id + "-no").toggle(!checked);
          new_attrs = {};
          els.each(function() {
            if (this.id) {
              new_attrs[this.id] = $(this).is(':checked');
            }
            return true;
          });
          return $.ajax({
